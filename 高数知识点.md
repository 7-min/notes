...
1. $\ \lim_\limits{x \rightarrow x_0}f(x) = \lim_{x \rightarrow x_0} g(x) = 0 $

	 2. $\ f(x) 和 g(x) 在 \mathring{U}(x_0,\delta) $ 内可导，且$\ g(x) \not= 0 $
	 2. $\ \lim_\limits{x \rightarrow x_0} \frac{f^{'}(x)}{g^{'}(x)} $ 存在 （或$\ = \infty$）` *注意，洛必达出来极限不存在不能证明原函数极限不存在`
